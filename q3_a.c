...
void check_transmission(System_State *sys) {
    while (sys->current_tokens > 0 && fifoqueue_size(sys->data_bucket) > 0) {
...
void event_packet_arrival(Simulation_Run_Ptr run, void *arg) {
    ...
    if (fifoqueue_size(sys->data_bucket) < sys->max_data_Bd) {
        fifoqueue_put(sys->data_bucket, (void *)sys);
    } else {
        sys->packets_lost++;
    }
    check_transmission(sys);
...
int main(int argc, char *argv[]) {
    /* requires 4 arguments: bt, bd, rate, seed */
    if (argc != 5) {
        return 1;
    }
    int Bt = atoi(argv[1]);
    ...
    double output_rate = (double)sys.packets_sent / SIM_END_TIME;

    /* output format: loss_rate,output_rate */
    printf("%.5f,%.2f\n", loss_rate, output_rate);
